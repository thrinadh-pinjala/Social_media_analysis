...
API_KEY = "AIzaSyCrzhOfOwbaqTZZF6uTMWdN1VdE1p1R4pQ"
...
def analyze_sentiment():
    try:
        ...
        for api_video in api_videos:
            ...
            mongo_video = mongo_video_map.get(video_id, {"video_id": video_id, "title": api_video.get("title", ""), "top_comments": []})
            ...
            likes = api_video.get("likes", 0)
            views = api_video.get("views", 1)
            total_comments = len(comments)
            
            engagement_rate = (likes + total_comments) / views
            view_to_like_ratio = likes / views
            
            engagement_metrics[api_video.get("title", "Unknown Video")] = {
                "engagement_rate": round(engagement_rate, 3),
                "view_to_like_ratio": round(view_to_like_ratio, 3),
            }
            ...
            video_sentiments.append({
                "video_title": api_video.get("title", "Unknown Video"),
                **sentiments,
